...
深交所对海外 IP 软拒（GitHub Actions 抓不到），所以让杭州 ECS 当"哨站"：
每周一、周四 18:07 抓沪深两所官方股票总市值，经 GitHub API 写回仓库 `data/cn_mcap.json`。
GitHub Actions 的周更校锚脚本（update_anchors.py）优先读这个文件来校 A股锚点。

## 安装（主人操作：登录 ECS 后整段粘贴，一次性）

登录服务器（阿里云控制台 → ECS → 远程连接，或本地 `ssh root@118.31.109.150`），把下面整段复制粘贴回车：
...
**成功标志**：最后一行打印出 `A股总市值 xxxxxx.x 亿元 ✓`（正常在 90万亿元上下，即 9e5 亿元量级）。
之后每周一、周四 18:07（服务器为北京时间）自动跑，日志在 `/root/us-stock-cnmcap.log`。

## 注意

- 依赖 `/root/.gh_token`（已存在，触发工作流用的同一把）。**token 到期换新时（约 2026-10），这个哨站自动跟着好，无需再动。**
- 若打印报错：把报错内容发给 Claude 即可，脚本改好后重新粘贴一遍就是更新。
- 验证闭环：粘贴成功后，让 Claude 手动跑一轮「每周自动校准市场规模锚点」工作流（或等下个周日），
  data/anchors_log.json 里 A股应显示「ECS哨站(沪深交易所官方)」。
