...
- 架构：阿里云ECS(118.31.109.150, 杭州, Asia/Shanghai时区) crontab 每30分钟(7,37)通过 GitHub API workflow_dispatch 触发 update-data.yml，替代不可靠的GitHub自带cron（后者保留为备胎，两个闹钟谁响都能开工，仓库有并发防撞）。
- 脚本 /root/us-stock-trigger.sh 读 /root/.gh_token（600权限，仅root可读）+ /root/.gh_payload（{"ref":"main"}）；日志 /root/us-stock-trigger.log。cron: `7,37 * * * 1-5` + `7,37 0-8 * * 6`（覆盖美股盘前盘中盘后北京时段）。
