...
- 美股盘前/盘后实时价：qt.gtimg.cn 常规字段停在昨收，但 f[9]买一/f[19]卖一在延长时段是实时盘口→取中间价（价差>6%视为异常弃用）；usSession() 用 America/New_York 时区判定盘前(4:00-9:30ET)/盘中/盘后(16:00-20:00ET)，自动处理夏令时；顶部标签显示 美股盘前/盘后/休市(昨收)。原定16:05验证任务的结论以此为准，可删除该定时任务。
