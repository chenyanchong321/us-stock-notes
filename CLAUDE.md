...
- index.html「实时行情」IIFE：页面每30秒经 `qt.gtimg.cn`（腾讯行情，script 标签 JSONP 免 CORS，charset=GBK）直连刷新**现价与当日涨跌**，并重算买点触达聚合条。覆盖 A股(sh/sz)、港股(hk五位码)、美股(usTICKER)、主要指数（映射表 IDX）；台/日/韩/OTC/商品/加密不覆盖，保持每日快照值。
- 分层职责：实时层管现价/当日；GitHub Actions 管历史高点/52周/PE/各周期涨跌等慢变量（cron 已错峰 :07/:37）。腾讯接口若失效，页面自动退回每日快照，不会坏。

## 视图切换器（2026-07-07 增）

- 顶部统计卡即视图切换：全景（观察标的）/ ⭐重点清单（买点标的，含"距买点"列，触达置顶）/ 今日涨幅榜Top30 / 跌幅榜Top30 / 回撤榜(>30%) / 板块热力（当日板块平均涨跌）。
