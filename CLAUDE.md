...
- 数量筛选 perf-pill：Top30/Top50/Top100/全部，仅当标的数超过档位才显示该档（小市场自动少档）；选中档 ≥ 总数时等效「全部」。**默认全部**（主人指定：排行榜本义是完整清单），记忆在 localStorage.mcapN（0=全部）；卡片角标固定「全表」，不写 TopN（避免误以为榜单只收录前30名）。
